...
import numpy as np
from data_preprocess.preprocess_smi_to_3d import smi2coords_3D, smi2coords_2D
from data_preprocess.basic import (
    csv_file_read,
    get_target_order,
...
def make_lmdb(smis, outputfilename):
    ...
    # print(dataset_smi) [[0, 0, '>>COc1cc([C@@H]2c3cc4c(cc3[C@@H](O[C@@H]3O[C@@H]5CO[C@@H](C)O[C@H]5[C@H](O)[C@H]3O)[C@H]3COC(=O)[C@H]23)OCO4)cc(OC)c1O']]
    env_new = lmdb.open(
        outputfilename,
        subdir=False,
        readonly=False,
    ...
    for i in tqdm(range(len(smis))):
        try:
            result = smis[i]
            raw_string = ">>"+str(result)
            #raw_string = str(result)+">>"+str(result)
...
if __name__ == "__main__":
    ...
    smis=["COc1cc([C@@H]2c3cc4c(cc3[C@@H](O[C@@H]3O[C@@H]5CO[C@@H](C)O[C@H]5[C@H](O)[C@H]3O)[C@H]3COC(=O)[C@H]23)OCO4)cc(OC)c1O","COc1cc(/C=C/CO)ccc1O"]
    outputfilename="ccc.lmdb"
    make_lmdb(
        smis=smis,

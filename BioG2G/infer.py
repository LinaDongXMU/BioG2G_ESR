...
from data_preprocess.lmdb_preprocess3 import make_lmdb
logging.basicConfig(
...
logger = logging.getLogger("BioG2G_cli.validate")
...
def init(args):
    ...
    # if args.task == "G2GT":
    #     state = G2GT_weight_reload(state)
...
def run(smiles, model_tuple, task_type='retrosynthesis', seed=42):
    ...
    a = make_lmdb(smiles,"../dataset/plus/infer.lmdb")
    try:
        task.load_dataset("infer", combine=False, epoch=1, task_cfg=args.task)
    except KeyError:
        raise Exception("Cannot find dataset: " + "infer.lmdb")
    # dataset_empty = task.load_empty_dataset(name=name, init_values=smiles, seed=seed)
    ...
    for i, sample in enumerate(itr):
        ...
        print(result) 
        with open("../dataset/plus/infer_result.txt", "a") as f:
            f.write(str(result))
        # return result
    # del lmdb
    os.remove("../dataset/plus/infer.lmdb")


def main(args):
    args.dict_name='dict_20231102.txt'
    model_tuple = init(args)
    with open("../dataset/plus/infer_input.txt", "r") as f:
        smiles = f.readlines()
    # retro_smiles = [
...
def cli_main():
    ...
    with open("../dataset/plus/args.txt", "w") as f:

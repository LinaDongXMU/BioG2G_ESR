# Copyright (c) DP Technology, Inc. and its affiliates.
#
...
if __version__ == "2.0.0":
    ...
    from .transformer_m import G2GTMTask

    logger = logging.getLogger(__name__)

    @register_task("G2GT_unimolv2")
    class G2GTUnimolv2Task(G2GTMTask):
        """Task for training transformer auto-encoder models."""

        def load_dataset(self, split, **kwargs):
            """Load a given dataset split."""
            # === inference mode special case ===
            if split == "infer":
                split_path = os.path.join(self.args.data, split + ".lmdb")
                if not os.path.exists(split_path):
                    raise FileNotFoundError(f"Inference LMDB not found: {split_path}")

                raw_dataset = LMDBDataset(split_path)
                sample_item = raw_dataset[0]
                keys = list(sample_item.keys())
                logger.info(f"[infer] Found keys in infer.lmdb: {keys}")

                # --- Determine SMILES key ---
                if "product_smiles" in sample_item:
                    smiles_key = "product_smiles"
                    product_smiles_kds = KeyDataset(raw_dataset, smiles_key)

                elif "rxn_smiles" in sample_item:
                    logger.info("[infer] Found 'rxn_smiles', converting to product_smiles...")

                    from unimol.data import KeyDataset as UnimolKeyDataset

                    class RxnToProductKeyDataset(UnimolKeyDataset):
                        """
                        A KeyDataset-like wrapper that extracts the product part from 'rxn_smiles'.
                        Inherits KeyDataset so NestedDictionaryDataset recognizes it as a Dataset.
                        """
                        def __init__(self, base):
                            # initialize KeyDataset to return the 'rxn_smiles' string
                            super().__init__(base, "rxn_smiles")

                        def __getitem__(self, idx):
                            rxn = super().__getitem__(idx)  # this returns the rxn_smiles string
                            if rxn is None:
                                return ""  # defensive
                            # typical reaction SMILES format: reactants>reagents>products
                            if ">" in rxn:
                                parts = rxn.split(">")
                                prod = [p for p in parts if p.strip() != ""][-1]
                            else:
                                prod = rxn
                            return prod.strip()

                    product_smiles_kds = RxnToProductKeyDataset(raw_dataset)

                elif "smiles" in sample_item:
                    smiles_key = "smiles"
                    product_smiles_kds = KeyDataset(raw_dataset, smiles_key)
                else:
                    raise KeyError(
                        f"No valid SMILES key found in inference dataset. Keys: {list(sample_item.keys())}"
                    )

                # Add mapping (atom indexing etc.)
                product_smiles_mapped = AddMapDataset(product_smiles_kds)
                product_graphormer = GraphormerDataset(
                    product_smiles_mapped,
                    use_stereoisomerism=self.args.use_stereoisomerism,
                )

                # dummy coordinates so Unimolv2Features works

                import numpy as np
                from unicore.data import FromNumpyDataset

                dummy_coords = [np.zeros((1, 3), dtype=np.float32) for _ in range(len(product_graphormer))]
                dummy_coord_dataset = FromNumpyDataset(dummy_coords)

                product_features = Unimolv2Features(
                    product_graphormer,
                    dummy_coord_dataset,
                    None,
                    is_train=False,
                    label_prob=1,
                    mid_prob=getattr(self.args, "mid_prob", 0.0),
                    mid_lower=getattr(self.args, "mid_lower", 0.0),
                    mid_upper=getattr(self.args, "mid_upper", 0.0),
                    noise=getattr(self.args, "noise_scale", 0.0),
                    seed=self.seed + 2,
                    N_vnode=self.args.N_vnode,
                )

                net_input = {"batched_data": product_features}
                target_dict = {"product_smiles": product_smiles_kds}
                self.datasets[split] = NestedDictionaryDataset(
                    {"net_input": net_input, "target": target_dict}
                )
                logger.info(
                    f"[infer] Loaded inference dataset from {split_path} with {len(raw_dataset)} samples."
                )
                return

            # === end inference special ===

            # Below: original training/validation logic
        ...
        def load_dataset_detail(
            ...
            target_dict = {"product_smiles": product_smiles_dataset}
            if flag_label_wanted or flag_label_smiles_wanted:

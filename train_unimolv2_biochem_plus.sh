#cd /vepfs/fs_users/yaolin/encoder_collections/BioG2G/unimol_v2_guolin-test-pretrain-0209
#pip install .
#cd /public/home/zjh/dln/NAG2G/NAG2G-chirality/BioG2G/BioG2G_model
[ -z "${MASTER_PORT}" ] && MASTER_PORT=10085
[ -z "${MASTER_IP}" ] && MASTER_IP=127.0.0.5
...
# [ -z "${data_path}" ] && data_path='/vepfs/fs_projects/BioG2G/dataset/BioG2G_/retro/USPTO50K_brief_20230227'
[ -z "${data_path}" ] && data_path='../dataset/plus'
...
# [ -z "${bpe_tokenizer_path}" ] && bpe_tokenizer_path="/vepfs/fs_users/yaolin/BioG2G/BioG2G_model/data_preprocess/token_preprocess/tokenizers/tokenizer_50k_230413_wosep"
# [ -z "${bpe_tokenizer_path}" ] && bpe_tokenizer_path="/vepfs/fs_users/yaolin/BioG2G/BioG2G_model/data_preprocess/token_preprocess/tokenizers/tokenizer_50k_230413_aftsep2"
...
[ -z "${max_seq_len}" ] && max_seq_len=1104

time=$(date "+%Y%m%d-%H%M%S")
echo "time" $time
task_name='BioG2G_G2GT_unimolv2_uspto_50k'
...
save_dir_="../outputs_biochem_plus/weights/${task_type}/$(date '+%Y-%m-%d')/${base_name}"
...
torchrun \
      ...
      $(which unicore-train) $data_path --user-dir ./BioG2G \
      --train-subset train --valid-subset valid,test \
      --num-workers ${num_workers} --ddp-backend=no_c10d \
      --task G2GT_unimolv2 --loss G2GT --arch BioG2G_G2GT --encoder-type unimolv2 --noise-scale ${noise_scale} --dict-name ${dict_name} \
      ${gpu_type} --fp16-init-scale 4 --fp16-scale-window 256 --tensorboard-logdir $save_dir/tsb \
      --log-interval 100 --log-format simple \
      --save-interval-updates 100000 --validate-interval-updates 100000 --keep-interval-updates 200 --no-epoch-checkpoints  \
      --save-dir $save_dir \

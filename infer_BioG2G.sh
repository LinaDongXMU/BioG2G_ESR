[ -z "${MASTER_PORT}" ] && MASTER_PORT=12349
[ -z "${MASTER_IP}" ] && MASTER_IP=127.0.0.2
...
[ -z "${beam_size}" ] && beam_size=20
[ -z "${beam_size_second}" ] && beam_size_second=5
[ -z "${beam_head_second}" ] && beam_head_second=2
# SimpleGenerator SequenceGeneratorBeamSearch SequenceGeneratorBeamSearch_test SequenceScoreGeneratorBeamSearch
...
# u50 uf g50 gf
if echo "${results_path}" | grep -q "unimolv2_uspto_50k"; then
...
torchrun \
      ...
      BioG2G/infer.py $data_path --user-dir ./BioG2G \
      --valid-subset test \
      --task ${task} --loss G2GT --arch BioG2G_G2GT --encoder-type ${encoder_type} \
      --seed $seed \
      --infer_step \
